...
{
    _global.sw = Number(Math.round(Stage.width));
    _global.sh = Number(Math.round(Stage.height));
    _mainMc._rszeFnc();
    _actvSctn._rszeFnc();
    _floGlryMc._rzeFnc();
    _vdoPlyrMc._rzeFnc();
...
setStage();
_mainMc = attachMovie("_mainMc", "_mainMc", this.getNextHighestDepth());
MovieClip.prototype._rtoImgPrto = function (_wdthPrto, _hghtPrto)
{
    if (sw / sh >= _wdthPrto / _hghtPrto)
    {
        this._width = sw;
        this._height = Number(sw / (_wdthPrto / _hghtPrto));
    }
    else
    {
        this._width = Number(sh * (_wdthPrto / _hghtPrto));
        this._height = sh;
    } // end else if
    this._x = Number((sw - this._width) / 2);
    this._y = Number((sh - this._height) / 2);
};
MovieClip.prototype._cntrAlgnW = function ()
{
    this._x = Math.round((sw - this._width) / 2);
    this._y = Math.round((sh - this._height) / 2);
};
MovieClip.prototype._cntrAlgn = function ()
{
    this._x = Math.round(sw / 2);
    this._y = Math.round(sh / 2);
};
